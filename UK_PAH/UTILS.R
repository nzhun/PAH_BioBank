...
merge_case_control<-function(gnomad_dat,PAH_CHD_dat){
  gnomad_dat$proband <- "."
  ...
  nm<-intersect(names(PAH_CHD_dat),names(gnomad_dat))
  dat<-rbind(PAH_CHD_dat[,nm],gnomad_dat[,nm])
  return(dat)
  #write.table(,
...
filter_case_VQSR <- function(dat,total_case,f){
 ...
 # dat<-dat[grep("^exonic|^splic",dat$Func.wgEncodeGencodeBasicV19,ignore.case = T),] 
  dat<-dat[grep(";|-",dat$Gene.refGene,invert=T),]
  dat<-dat[which(dat$FILTER=="PASS"),]
  
  #dat<-VQSR_filter(dat) 
  dat<-dat[which( dat$gnmad_genome_15=="." | (as.numeric(dat$gnmad_genome_15)>0.9 ) ),] # &  as.numeric(dat$gnomad_COV15)>0.75
  cnt<-c(dim( dat)[1]/total_case,
         dim( dat[grep("^synony", dat$ExonicFunc.refGene),])[1]/total_case,
         dim( dat[grep("^nonsynony", dat$ExonicFunc.refGene),])[1]/total_case,
         dim(dat[grep("non",dat$ExonicFunc.refGene,invert = T),])[1]/total_case) 
 ...
 # dat<-map_filter(dat)
  ...
  # print(cnt)
  # 
  dat<-dat[which( as.numeric(dat$AF)< 40*f &  (as.numeric(dat$AC)/as.numeric(dat$AN)) < 40*f),]
  cnt<-c(dim( dat)[1]/total_case,
         dim( dat[grep("^synony", dat$ExonicFunc.refGene),])[1]/total_case,
         dim( dat[grep("^nonsynony", dat$ExonicFunc.refGene),])[1]/total_case,
         dim(dat[grep("non",dat$ExonicFunc.refGene,invert = T),])[1]/total_case) 
  
  filter_rs<-rbind(filter_rs,c("cohorts 0.004 ",cnt))
  print("local cohort 10-2")
  print(cnt)
  
  #gq<-unlist(lapply(dat$Genotype,FUN = function(x){unlist(strsplit(x,":"))[4]}))  ## For RGN
  # dat$GQ<-gq
  rem<-which(as.numeric(dat$GQ_Ind)< 70)
  if(length(rem)>0){
    PAH_CHD_dat<-dat[-rem,]  ## only for RGN
  }else{
    PAH_CHD_dat<-dat
  ...
  filter_rs<-rbind(filter_rs,c("MQ 40 ",cnt))
  ...
  #   PAH_CHD_dat<-PAH_CHD_dat[-rm,]
  # }
  remove<- which( PAH_CHD_dat$AD< 6  |PAH_CHD_dat$DP_ind >100)  #PAH_CHD_dat$DP_ind<10 | PAH_CHD_dat$AB< 0.3 |
  if(length(remove)>0){
    PAH_CHD_dat<-PAH_CHD_dat[-remove,] ## only for RGN
  }
  ...
  #   PAH_CHD_dat<-PAH_CHD_dat[-remove,] ## only for RGN
  # }
  PAH_CHD_dat<- PAH_CHD_dat[which( as.numeric(PAH_CHD_dat$AB) > 0.2),]
  ...
  # PAH_CHD_dat<- PAH_CHD_dat[which( as.numeric(PAH_CHD_dat$VQSLOD) > -4),]
   rm<- which( PAH_CHD_dat$InbreedingCoeff < -0.8  & PAH_CHD_dat$Is_indel == T)
   if(length(rm) >0){
     PAH_CHD_dat<- PAH_CHD_dat[-rm,]
   }
  
  snv_fail<-c(which( (as.numeric(PAH_CHD_dat$QD) < 1| as.numeric(PAH_CHD_dat$SOR) > 4)  ), 
              which(as.numeric(PAH_CHD_dat$ReadPosRankSum) < -2| as.numeric(PAH_CHD_dat$MQRankSum ) < -2 ),
              which(PAH_CHD_dat$BaseQRankSum!="." & (as.numeric(PAH_CHD_dat$BaseQRankSum) < -2 | as.numeric(PAH_CHD_dat$BaseQRankSum) > 2))
              
              ) #| PAH_CHD_dat$SOR > 3
  # # snv_gq_fail<-c() #intersect(which(gq<70),which(is_indel==F))
  remove<- unique(snv_fail) #intersect(unique(snv_fail),which(PAH_CHD_dat$Is_indel==F)) #(c(indel_fail,snv_fail,snv_gq_fail))
  if(length(remove)>0){
    PAH_CHD_dat<-PAH_CHD_dat[-remove,] ## only for RGN
  }
...
filter_gnomad<-function(dat,f){
  ...
  subdat<-subdat[grep("^exonic|^splic",subdat$Func.refGene,ignore.case = T),] 
 # subdat<-subdat[grep("^exonic|^splic",subdat$Func.wgEncodeGencodeBasicV19,ignore.case = T),] 
  subdat<-subdat[grep("PASS",subdat$FILTER,invert = F),]
  subdat<-subdat[grep(";|-",subdat$Gene.refGene,invert=T),]
  ...
  gfilter_rs<-rbind(gfilter_rs,c("PASS",cnt))
  subdat<-subdat[which(as.numeric(subdat$DP_15) >=0.9|subdat$DP_15=="."),]
...
#  subdat<-map_filter(subdat)
  ...
  subdat <- filter_allfreq_local(subdat,0.001,f)  ## Max gnomad exome <10^-4 ## Max gnomad exome <10^-4
  cnt<-c(
    sum(subdat$AC_NFE[grep("^synony",subdat$ExonicFunc.refGene)])/total_control,
    sum(subdat$AC_NFE[grep("^nonsynony",subdat$ExonicFunc.refGene)])/total_control,
    sum(subdat$AC_NFE[grep("non",subdat$ExonicFunc.refGene,invert = T)])/total_control)
  ...
  subdat <- subdat[which(as.numeric(subdat$AF)< 40*f & as.numeric(subdat$AF_NFE)< 40*f  ),] 
  cnt<-c(
    sum(subdat$AC_NFE[grep("^synony",subdat$ExonicFunc.refGene)])/total_control,
    sum(subdat$AC_NFE[grep("^nonsynony",subdat$ExonicFunc.refGene)])/total_control,
    sum(subdat$AC_NFE[grep("non",subdat$ExonicFunc.refGene,invert = T)])/total_control)
  ...
  rm<-which(subdat$BaseQRankSum!="." & (as.numeric(subdat$BaseQRankSum) < -4 | as.numeric(subdat$BaseQRankSum) > 4))
  if(length(rm) >0) {
    subdat<-subdat[-rm,]
  }

 rm<-intersect(which(subdat$BaseQRankSum!="." & (as.numeric(subdat$BaseQRankSum) < -4 | as.numeric(subdat$BaseQRankSum) > 4) ), grep('non',subdat$ExonicFunc.refGene,invert = T))
  if(length(rm) >0) {
    subdat<-subdat[-rm,]
  }
